...
def extract_merchants_from_descriptions(description):
    """Extract merchant name and category from a single transaction description using LLM.

    Args:
        description (str): A single transaction description string.

    Returns:
        dict: Dict with merchant_name and category.
              Format: {'merchant_name': 'NIKE', 'category': 'Shopping'}
              merchant_name may be None for transfers or unidentifiable transactions.
    """

    # desc_list = "\n".join([f"{i + 1}. {desc}" for i, desc in enumerate(descriptions)]) # Used if batch processing description
    ...
    Return ONLY a JSON object with merchant_name and category.
    Example: {{"merchant_name": "Caffe Nero", "category": "Coffee"}}
    ...
    response_text = response.choices[0].message.content.strip()
    # print(response_text)
    merchant = json.loads(response_text)
