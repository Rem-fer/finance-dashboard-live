...
def categorise_step_two():
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT DISTINCT description FROM finance.transactions WHERE category IS NULL")
        rows = cursor.fetchall()
        print(f"Categorising {len(rows)} transactions with LLM...")

        for row in rows:
            try:
                categorised = extract_merchants_from_descriptions(row[0]) # Row[0] Because cursor returns tuple
                ...
                cursor.execute(
                    """INSERT INTO finance.merchants
                       ...
                       WHERE finance.merchants.merchant_name IS NULL
                       AND finance.merchants.category IS NULL""",
                    (
                        row[0],
                ...
                # Then update the transaction table with that description
                cursor.execute(
                    """UPDATE finance.transactions
                       SET category = %s
                       WHERE description = %s
...
def categorise_transactions():
    # Debug: save uncategorised transaction IDs before categorisation
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT transaction_id FROM finance.transactions WHERE category IS NULL")
    uncategorised_ids = [row[0] for row in cursor.fetchall()]
    print(f"Uncategorised before: {len(uncategorised_ids)}")
    conn.close()
    ...
    # Debug: check categories for previously uncategorised transactions
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("""
        SELECT transaction_id, description, category 
